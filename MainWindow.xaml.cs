...
public partial class MainWindow : Window
{
    ...
    private Dictionary<TabViewModel, string> _mediaArtistCache = new();

    private async Task UpdateTabMediaMetadataAsync(TabViewModel tab, Controls.BrowserView browser)
    {
        if (browser.MainWebView?.CoreWebView2 == null || browser.IsProcessCrashed) return;
        try
        {
            string script = @"(() => {
                let title = null;
                ...
                const ytc = document.querySelector('ytd-channel-name a, ytd-video-owner-renderer a, #owner-name a, .ytp-title-channel-name');
                if (ytc) artist = ytc.innerText.trim();
                if (!artist) {
                    const sp = document.querySelector('[data-testid=""creator-link""], [data-testid=""now-playing-widget""] a[href^=""/artist""]');
                    if (sp) artist = sp.innerText.trim();
                }
                if (!artist) {
                    const sc = document.querySelector('.playbackSoundBadge__titleContext, .soundTitle__username');
                    if (sc) artist = sc.innerText.trim();
                }
                if (!artist) {
                    const meta = document.querySelector('meta[name=""author""], meta[property=""og:audio:artist""], meta[property=""music:musician""], meta[name=""music:musician""]');
                    if (meta) artist = meta.content.trim();
                }
                if (!artist && title && (title.includes(' - ') || title.includes(' – ') || title.includes(' — '))) {
                    const sep = title.includes(' — ') ? ' — ' : (title.includes(' – ') ? ' – ' : ' - ');
            ...
            if (!string.IsNullOrEmpty(result) && result != "null")
            {
                ...
                if (!string.IsNullOrEmpty(unescaped))
                {
                    ...
                    string parsedTitle = root.TryGetProperty("title", out var tp) && tp.ValueKind == System.Text.Json.JsonValueKind.String ? tp.GetString() ?? "" : "";
                    string parsedArtist = root.TryGetProperty("artist", out var ap) && ap.ValueKind == System.Text.Json.JsonValueKind.String ? ap.GetString() ?? "" : "";
    ...
    private DispatcherTimer? _batteryFlashTimer;
    private bool             _batteryFlashOn   = false;
    private const int        BatteryRedThreshold = 20;
    ...
    private void MainWindow_StateChanged(object? sender, EventArgs e)
    {
        if (WindowState == WindowState.Minimized
            && SettingsService.Current.MediaWidgetAutoShowOnMinimize
            && _allTabs.Any(t => t.IsPlayingAudio))
        {
            Dispatcher.BeginInvoke(new Action(() =>
            {
                if (WindowState != WindowState.Minimized) return;
                if (!_allTabs.Any(t => t.IsPlayingAudio)) return;

                OpenMusicControlsPopup();
                if (SettingsService.Current.MediaWidgetAutoShowMinimized && _mediaWidgetWindow != null)
                {
                    _mediaWidgetWindow.ShowInTaskbar = true;
                    _mediaWidgetWindow.WindowState    = WindowState.Minimized;
                }
    ...
    public MainWindow(string? startUrl = null, bool isWebApp = false)
    {
        ...
        {
            if (CurrentBrowser?.MainWebView != null && !CurrentBrowser.IsProcessCrashed && CurrentBrowser.MainWebView.Source != null)
            {
                string title;
                try { title = CurrentBrowser.MainWebView.CoreWebView2?.DocumentTitle ?? ""; }
                catch (InvalidOperationException) { title = ""; }
    ...
    newBrowser.MainWebView.NavigationCompleted += async (s, e) =>
{
    try
    {
        ...
        try
        {
            if (!newBrowser.IsProcessCrashed && newBrowser.MainWebView.CoreWebView2 != null)
            {
                string js = @"(() => {
        ...
        if (!_audioHooked && !newBrowser.IsProcessCrashed && newBrowser.MainWebView.CoreWebView2 != null)
        {
            _audioHooked = true;
            ...
            newBrowser.MainWebView.CoreWebView2.DocumentTitleChanged += async (sender2, args2) =>
            {
                try 
                {
                    ...
                    {
                        _ = UpdateTabMediaMetadataAsync(newTab, newBrowser);
...
");
    ...
    private void DoCloseTab(TabViewModel tabToClose)
    {
        ...
        _mediaArtistCache.Remove(tabToClose);
        ...
        if (isPlaying)
        {
            ...
            try
            {
                if (browser.MainWebView?.CoreWebView2 != null)
                {
                    ...
                    _ = UpdateTabMediaMetadataAsync(tab, browser);
                }
            }
            catch { }
    ...
    private void StartColorAnimation(TabViewModel tab)
    {
        ...
        timer.Tick += (s, e) =>
        {
            ...
            if (tab.IsPlayingAudio)
                MediaBridge.PublishGradient(c0, c1, c2, midOffset);
        };
    ...
    {
        System.Diagnostics.Debug.WriteLine("[RCLICK] BtnMediaBack_RightClick fired.");
        e.Handled = true;
        if (sender is Button btn && btn.DataContext is TabViewModel tab && _tabViews.TryGetValue(tab, out var browser))
        {
            var core = browser.MainWebView?.CoreWebView2;
            ...
            await core.ExecuteScriptAsync(@"(() => {
                const b = document.querySelector('.ytp-prev-button,[data-testid=""previous-button""],.skipControl__previous,tp-yt-paper-icon-button#previous-button,#previous-button,.previous-button,[aria-label=""Previous""],[aria-label=""Previous track""],[title=""Previous""]');
                if (b) { b.click(); return; }
    ...
    private async void BtnMediaForward_RightClick(object sender, MouseButtonEventArgs e)
    {
        ...
        if (sender is Button btn && btn.DataContext is TabViewModel tab && _tabViews.TryGetValue(tab, out var browser))
        {
            ...
            await core.ExecuteScriptAsync(@"(() => {
                const b = document.querySelector('.ytp-next-button,[data-testid=""next-button""],.skipControl__next,tp-yt-paper-icon-button#next-button,#next-button,.next-button,[aria-label=""Next""],[aria-label=""Next track""],[title=""Next""]');
                if (b) { b.click(); return; }
...
");
    ...
    {
        bool overButton = e.OriginalSource is DependencyObject d && FindAncestor<Button>(d) != null;
        System.Diagnostics.Debug.WriteLine($"[RCLICK] TabHeader preview fired. overButton={overButton} source={e.OriginalSource}");
    ...
    private void InitWidget()
    {
...

        {
            ...
            {
                if (string.IsNullOrWhiteSpace(_initCity))
                {
                    _initCity = AutoDetectCityAsync().GetAwaiter().GetResult();
                    if (!string.IsNullOrWhiteSpace(_initCity))
                    {
                        Dispatcher.Invoke(() =>
                        {
                            SettingsService.Current.WidgetWeatherCity = _initCity;
                            SettingsService.Save();
                            TxtWeatherCity.Text = _initCity;
                        });
                    }
                }
                ...
                if (!string.IsNullOrWhiteSpace(_initCity))
                {
                    ...
                    Dispatcher.Invoke(() => StartWeatherRefreshTimer(_initCity));
                }
            });
        }

        Services.BatteryBridge.Start();
        Services.BatteryBridge.Updated += () => Dispatcher.Invoke(() =>
        {
            if (CurrentWidgetMode() == "Battery")
            {
                TxtWidget.Text = GetWidgetText();
    ...
    private async void VideoWidgetTimer_Tick(object? sender, EventArgs e)
    {
        ...
        if (vidTab != null)
        {
            ...
            {
                _ = UpdateTabMediaMetadataAsync(vidTab, vidBv);
            }
        ...
        {
            _ = UpdateTabMediaMetadataAsync(_aoTab, _aoBv);
        }
    ...
    private string GetBatteryWidgetText()
    {
        if (!Services.BatteryBridge.HasBattery) return "🔋  N/A";
        string icon = Services.BatteryBridge.IsCharging ? "⚡" : "🔋";
        string time = Services.BatteryBridge.TimeRemaining is TimeSpan t
            ? FormatTimeSpan(t)
            : "--:--";
        return $"{icon}  {Services.BatteryBridge.Percent}%  ·  {time}";
    }

    private static Color BatteryColorFor(int percent)
    {
        var green  = C(0x2ecc71);
        var yellow = C(0xf1c40f);
        var red    = C(0xe74c3c);
        if (percent >= 50) return LerpColor(yellow, green, (percent - 50) / 50.0);
        return LerpColor(red, yellow, percent / 50.0);
    }

    private void UpdateBatteryWidgetVisual()
    {
        if (BatteryFillRect == null) return;

        if (!Services.BatteryBridge.HasBattery)
        {
            BatteryFillRect.Visibility = Visibility.Collapsed;
            StopBatteryFlash();
            return;
        }

        int percent = Math.Clamp(Services.BatteryBridge.Percent, 0, 100);
        double containerW = (HeaderWidgetBorder.ActualWidth > 0 ? HeaderWidgetBorder.ActualWidth : _widgetDefaultWidth) - 2;
        var color = BatteryColorFor(percent);

        BatteryFillRect.Visibility = Visibility.Visible;
        BatteryFillRect.Width      = containerW * (percent / 100.0);
        BatteryFillRect.Background = new SolidColorBrush(Color.FromArgb(0x55, color.R, color.G, color.B));

        if (percent <= BatteryRedThreshold && !Services.BatteryBridge.IsCharging)
            StartBatteryFlash();
        else
            StopBatteryFlash();
    }

    private void StartBatteryFlash()
    {
        if (_batteryFlashTimer != null) return;
        _batteryFlashOn    = false;
        _batteryFlashTimer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(600) };
        _batteryFlashTimer.Tick += (s, e) =>
        {
            _batteryFlashOn = !_batteryFlashOn;
            BatteryFillRect.Opacity = _batteryFlashOn ? 0.35 : 1.0;
        };
        _batteryFlashTimer.Start();
    }

    private void StopBatteryFlash()
    {
        if (_batteryFlashTimer == null) return;
        _batteryFlashTimer.Stop();
        _batteryFlashTimer = null;
    ...
    private void RefreshWidgetDisplay()
    {
        ...
        var _mediaActiveTab = _mediaVidTab ?? _mediaAudTab ?? _allTabs.FirstOrDefault(t => t.HasEverPlayedAudio);
        string activeArtist = _mediaActiveTab != null && _mediaArtistCache.TryGetValue(_mediaActiveTab, out var art) ? art : "";
        string activeHost = "";
        if (_mediaActiveTab != null && !string.IsNullOrEmpty(_mediaActiveTab.Url) && Uri.TryCreate(_mediaActiveTab.Url, UriKind.Absolute, out var _mediaUri))
            activeHost = _mediaUri.Host;
        MediaBridge.Publish(
            _mediaActiveTab != null ? (_mediaActiveTab.CleanMediaTitle ?? _mediaActiveTab.Title) : "",
            _mediaActiveTab?.IsPlayingAudio ?? false,
            _mediaActiveTab?.IsMuted ?? false,
            _mediaActiveTab?.HasVideo ?? false,
            _mediaActiveTab != null,
            _mediaActiveTab?.IsAudioOnlyMode ?? false,
            activeArtist,
            activeHost,
    ...
    private static readonly HttpClient _widgetHttp = new(new WeatherRetryHandler()) { Timeout = TimeSpan.FromSeconds(30) };

private sealed class WeatherRetryHandler : DelegatingHandler
{
    public WeatherRetryHandler() : base(new HttpClientHandler()) { }
    ...
    protected override async Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
    {
        ...
        {
            if (attempt > 0) await Task.Delay(500 * attempt, cancellationToken);
            using var cts = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken);
            cts.CancelAfter(TimeSpan.FromSeconds(8));
            var attemptRequest = new HttpRequestMessage(request.Method, request.RequestUri);
            foreach (var h in request.Headers) attemptRequest.Headers.TryAddWithoutValidation(h.Key, h.Value);
            try
            {
                var response = await base.SendAsync(attemptRequest, cts.Token);
                int code = (int)response.StatusCode;
                if (code < 500 && code != 429) return response;
                last = new HttpRequestException($"HTTP {code}");
                response.Dispose();
            }
    ...
    private async Task FetchWeatherAsync(string city)
    {
        ...
        for (int attempt = 0; attempt < 2; attempt++)
        {
            try
            {
                ...
                if (raw.Length > 0 && !raw.StartsWith("<") && !raw.StartsWith("Unknown"))
                {
                    ...
                    WeatherBridge.Publish(_weatherCache, _weatherWmoCode, city);
                    Dispatcher.Invoke(RefreshWidgetDisplay);
                    return;
                }
            }
        ...
        try
        {
            ...
            if (geoDoc.RootElement.TryGetProperty("results", out var geoResults) && geoResults.GetArrayLength() > 0)
            {
                ...
                WeatherBridge.Publish(_weatherCache, _weatherWmoCode, name);
    ...
    private IEnumerable<UIElement> BuildCurrentConditionsRows()
    {
        ...
        var emoji = new FontFamily("Segoe UI Emoji");
        var iconBrush = new SolidColorBrush(WeatherBridge.ThemeAccentSubtle);
        var labelBrush = new SolidColorBrush(WxMuted);

        TextBlock Icon(string glyph) => new TextBlock
        {
            Text = glyph, FontSize = 13, FontFamily = emoji, Foreground = iconBrush,
        ...
        Border Wrap(Grid g, string tip)
        {
            var row = new Border
            {
                Padding = new Thickness(4, 2, 4, 2), CornerRadius = new CornerRadius(3),
                Background = Brushes.Transparent, Child = g
            };
            if (!string.IsNullOrEmpty(tip)) row.ToolTip = tip;
            row.MouseEnter += (_, _) => row.Background = new SolidColorBrush(Color.FromArgb(0x1A, 0xFF, 0xFF, 0xFF));
            row.MouseLeave += (_, _) => row.Background = Brushes.Transparent;
            return row;
        }

        Border Header(UIElement icon, string text, FontWeight weight)
        {
            var g = Cols();
            Grid.SetColumn(icon, 0);
            g.Children.Add(icon);
            var t = Val(text, null, weight);
            Grid.SetColumn(t, 1);
        ...
        Border Row(UIElement? icon, string label, UIElement value, string tip)
        {
            var g = Cols();
            if (icon != null)
            {
            ...
            var l = new TextBlock
            {
                Text = label, FontSize = 12, FontFamily = mono, Foreground = labelBrush,
        ...
        rows.Add(Header(Icon("📍"), _cachedWxLoc, FontWeights.SemiBold));
        rows.Add(Header(WeatherIconBlock(_weatherWmoCode, 13), WmoCodeToDescription(_weatherWmoCode), FontWeights.Normal));
        rows.Add(new Border { Height = 8 });

        rows.Add(Row(Icon("🌡"), "Temp", Val($"{_cachedWxTemp:F1}°C  (feels {_cachedWxFeelsLike:F1}°C)"), ""));
        rows.Add(Row(null, "Hi / Lo", Val($"{_cachedWxTempMax:F1}°C  /  {_cachedWxTempMin:F1}°C"), ""));
        rows.Add(Row(Icon("💧"), "Humidity", Val($"{_cachedWxHumidity}%"), WxHumidityComfort(_cachedWxHumidity)));
        rows.Add(Row(Icon("🌬"), "Wind",
            Val($"{_cachedWxWindSpd:F0} km/h {WindDirectionToString(_cachedWxWindDir)}" +
                (_cachedWxWindGust > _cachedWxWindSpd + 5 ? $"  (gusts {_cachedWxWindGust:F0})" : "")),
            WxBeaufort(_cachedWxWindSpd)));
        rows.Add(Row(Icon("🌧"), "Precip",
            Val($"{_cachedWxPrecip:F1} mm now  |  {_cachedWxDailyPrecip:F1} mm today  ({_cachedWxRainChance:F0}% chance)"), ""));
        rows.Add(Row(Icon("🔍"), "Visibility",
            Val(_cachedWxVisM >= 1000 ? $"{_cachedWxVisM / 1000.0:F1} km" : $"{_cachedWxVisM:F0} m"), ""));
        rows.Add(Row(Icon("⏱"), "Pressure", Val($"{_cachedWxPressure:F0} hPa"), WxPressureTip(_cachedWxPressure)));
        rows.Add(Row(Icon("☀"), "UV Index",
            Val($"{_cachedWxUv:F0}  {WxUvLevel(_cachedWxUv)}", new SolidColorBrush(WxUvColor(_cachedWxUv))),
            WxUvProtection(_cachedWxUv)));

        var sun = new StackPanel { Orientation = Orientation.Horizontal };
        sun.Children.Add(Val(_cachedWxSunrise));
        var sunsetIcon = Icon("🌇");
        sunsetIcon.Margin = new Thickness(18, 1, 6, 0);
        sun.Children.Add(sunsetIcon);
        sun.Children.Add(Val("Sunset " + _cachedWxSunset));
        rows.Add(Row(Icon("🌅"), "Sunrise", sun, ""));
    ...
    private void OpenMusicControlsPopup()
    {
        ...
        Style MakePillStyle(double cornerRadius)
        {
            ...
            borderFactory.SetValue(Border.BorderBrushProperty, pillBorder);
            ...
            hoverTrigger.Setters.Add(new Setter(Border.BackgroundProperty, hoverBg, "PillBorder"));
        ...
        var ctrlPillStyle = MakePillStyle(17);
        var widePillStyle  = MakePillStyle(15);

        Button MakeCtrlBtn(string symbol, string tooltip) => new Button
        {
            Content  = symbol,
            Width    = 38,
            Height   = 34,
        ...
        {
            const string prevScript =
                "(() => { const b = document.querySelector(" +
                "'.ytp-prev-button,[data-testid=\"previous-button\"],.skipControl__previous," +
                "tp-yt-paper-icon-button#previous-button,#previous-button,.previous-button," +
                "[aria-label=\"Previous\"],[aria-label=\"Previous track\"],[title=\"Previous\"]'); " +
                "if(b){b.click();}else{const m=document.querySelector('video,audio');if(m)m.currentTime=0;}})()";
            const string nextScript =
                "(() => { const b = document.querySelector(" +
                "'.ytp-next-button,[data-testid=\"next-button\"],.skipControl__next," +
                "tp-yt-paper-icon-button#next-button,#next-button,.next-button," +
                "[aria-label=\"Next\"],[aria-label=\"Next track\"],[title=\"Next\"]'); " +
                "if(b){b.click();}else{const m=document.querySelector('video,audio');if(m&&isFinite(m.duration))m.currentTime=m.duration-0.1;}})()";

            btnPrev.Click += async (s, e) =>
            {
                e.Handled = true;
                await (browser.MainWebView?.CoreWebView2?.ExecuteScriptAsync(prevScript) ?? Task.CompletedTask);
            };
            ...
            btnBack.PreviewMouseRightButtonUp += async (s, e) =>
            {
                e.Handled = true;
                await (browser.MainWebView?.CoreWebView2?.ExecuteScriptAsync(prevScript) ?? Task.CompletedTask);
            };
            ...
            {
                e.Handled = true;
                await (browser.MainWebView?.CoreWebView2?.ExecuteScriptAsync(nextScript) ?? Task.CompletedTask);
            };
            btnFwd.MouseRightButtonUp += async (s, e) =>
            {
                e.Handled = true;
                await (browser.MainWebView?.CoreWebView2?.ExecuteScriptAsync(nextScript) ?? Task.CompletedTask);
            };
    ...
    private static double WxScale(Window w) => w.Tag is true ? 1.0 : 1.12;
    private static double WxPaneWidth(Window w) => w.Tag is true ? 380 : 340;
    private static double WxClosedWidth(Window w) => 400 * WxScale(w);
    private static double WxOpenWidth(Window w) => 400 * WxScale(w) + WxPaneWidth(w);
    private static double WxFavPaneWidth(Window w) => w.Tag is true ? 240 : 200;
    private static double WxTotalWidth(Window w, bool leftOpen, bool rightOpen) =>
        WxClosedWidth(w) + (leftOpen ? WxFavPaneWidth(w) : 0) + (rightOpen ? WxPaneWidth(w) : 0);
    ...
    private static string WxUvLevel(double uv) =>
    ...
    private static void WxTabStyle(Button btn, bool active)
    {
        btn.Foreground      = new SolidColorBrush(active ? WeatherBridge.ThemeAccent : WxTabInactive);
        btn.BorderBrush     = new SolidColorBrush(active ? WxPillOpaque : Colors.Transparent);
    ...
    private void OpenWeatherDetailPopup()
    {
        ...
        void StartPopupAnim()
        {
            ...
            popupAnimTimer.Tick += (_, _) =>
            {
                ...
                c0.A = (byte)(46 * fa); c1.A = (byte)(30 * fa); c2.A = (byte)(46 * fa);
        ...
        var tabBar = new StackPanel { Orientation = Orientation.Horizontal, Background = new SolidColorBrush(Color.FromArgb(0x50, 0x00, 0x00, 0x00)) };
        ...
        var favCol   = new ColumnDefinition { Width = new GridLength(0) };
        ...
        Grid.SetColumn(hourlyScroll, 2);

        var favScroll = new ScrollViewer
        {
            VerticalScrollBarVisibility = ScrollBarVisibility.Auto,
            Visibility = Visibility.Collapsed,
            BorderBrush = new SolidColorBrush(WxBorder),
            BorderThickness = new Thickness(0, 0, 1, 0)
        };
        Grid.SetColumn(favScroll, 0);
        ...
        var refreshBtn = AccentButton("↻  Refresh", WxButtonBg, WxButtonBorder, 110);
        ...
        var favHolder = new StackPanel { Margin = new Thickness(8) };
        favScroll.Content = favHolder;
        bool favExpanded = false;

        DateTime paneDate = DateTime.Today;
        int paneCode = 0;
        double paneMax = 0, paneMin = 0;
        bool paneUseLive = true;

        async Task LoadCityAsync(string nc)
        {
            if (string.IsNullOrEmpty(nc)) return;
            cityInput.Text = nc;
        ...
        {
            favHolder.Children.Clear();
            var addBtn = AccentButton("★  Add current city", WxButtonBg, WxButtonBorder, 170);
            addBtn.HorizontalAlignment = HorizontalAlignment.Stretch;
            addBtn.Margin = new Thickness(0, 0, 0, 8);
            addBtn.Click += (_, _) =>
            {
                string nc = cityInput.Text.Trim();
                if (string.IsNullOrEmpty(nc)) return;
                var list = SettingsService.Current.WeatherFavoriteCities;
                if (!list.Contains(nc, StringComparer.OrdinalIgnoreCase))
                {
                    list.Add(nc);
                    SettingsService.Save();
                    RebuildFavorites();
            ...
            favHolder.Children.Add(addBtn);

            foreach (var fc in SettingsService.Current.WeatherFavoriteCities)
            {
                var row = new Border
                {
                    Background = new SolidColorBrush(WxSurface),
                    BorderBrush = new SolidColorBrush(WxBorder),
                    BorderThickness = new Thickness(1),
                ...
                var label = new TextBlock
                {
                    Text = fc, FontSize = 12, FontFamily = new FontFamily("Consolas"),
                ...
                row.MouseEnter += (_, _) => row.Background = new SolidColorBrush(Color.FromArgb(0x30, 0xFF, 0xFF, 0xFF));
                row.MouseLeave += (_, _) => row.Background = new SolidColorBrush(WxSurface);
                row.MouseLeftButtonUp += async (_, e) =>
                {
                    if (e.OriginalSource == removeBtn)
                    {
                        SettingsService.Current.WeatherFavoriteCities.Remove(fc);
                        SettingsService.Save();
                        RebuildFavorites();
                        return;
                    }
                    await LoadCityAsync(fc);
                };
                favHolder.Children.Add(row);
            }
        }
        RebuildFavorites();
        ...
        async Task LoadTodayHourly()
        {
            ...
            hourlyHolder.Children.Add(new TextBlock
            {
                Text = "Loading…", Foreground = new SolidColorBrush(WxMuted), FontSize = 11,
                Margin = new Thickness(2, 6, 0, 6)
            });
            string paneCity = cityInput.Text.Trim();
            await EnsureWeatherGeoAsync(paneCity);
            var dayDate = paneDate;
            var hourlyTask = FetchWeatherHourlyAsync(dayDate);
            var uvTask = FetchWeatherHourlyUvAsync(dayDate);
            await Task.WhenAll(hourlyTask, uvTask);
            if (dayDate != paneDate) return;
            if (paneUseLive && dayDate == DateTime.Today)
            {
                if (_weatherWmoCode >= 0) paneCode = _weatherWmoCode;
                paneMax = _cachedWxTempMax;
                paneMin = _cachedWxTempMin;
            }
            hourlyHolder.Children.Clear();
            BuildDayPane(hourlyHolder, dayDate, paneCode, paneMax, paneMin, hourlyTask.Result, uvTask.Result);
        }
        ...
        cityInput.KeyDown += async (s, e) =>
        {
            if (e.Key == Key.Return)
            {
                await LoadCityAsync(cityInput.Text.Trim());
            }
        };
        ...
        cityInput.Background = new SolidColorBrush(Color.FromArgb(0x70, 0x00, 0x00, 0x00));
        cityInput.Foreground = Brushes.White;
        cityInput.BorderBrush = new SolidColorBrush(WxBorder);
        cityInput.CaretBrush = new SolidColorBrush(WeatherBridge.ThemeAccent);
        cityInput.FontFamily = new FontFamily("Consolas");
        refreshBtn.Margin = new Thickness(0, 12, 0, 0);

        var bodyGrid = new Grid();
        bodyGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        bodyGrid.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(0) });
        bodyGrid.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        bodyGrid.RowDefinitions.Add(new RowDefinition { Height = GridLength.Auto });
        var calendarHolder = new StackPanel { Margin = new Thickness(0, 6, 0, 0) };
        Grid.SetColumn(conditionsHolder, 0);
        Grid.SetRow(conditionsHolder, 0);
        Grid.SetColumn(calendarHolder, 0);
        Grid.SetRow(calendarHolder, 1);
        bodyGrid.Children.Add(conditionsHolder);
        bodyGrid.Children.Add(calendarHolder);
        currentInner.Children.Add(bodyGrid);

        bool nowSide = false;
        void ArrangeNow()
        {
            bool useSide = leftScroll.ActualWidth >= 570;
            if (useSide == nowSide) return;
            nowSide = useSide;
            bodyGrid.ColumnDefinitions[1].Width = useSide ? new GridLength(290) : new GridLength(0);
            Grid.SetColumn(calendarHolder, useSide ? 1 : 0);
            Grid.SetRow(calendarHolder, useSide ? 0 : 1);
            calendarHolder.Margin = useSide ? new Thickness(12, 0, 0, 0) : new Thickness(0, 6, 0, 0);
        }
        leftScroll.SizeChanged += (_, _) => ArrangeNow();

        var monthAnchor = new DateTime(DateTime.Today.Year, DateTime.Today.Month, 1);
        void ReloadCalendar()
        {
            string cityNow = cityInput.Text.Trim();
            if (string.IsNullOrEmpty(cityNow)) return;
            _ = BuildMonthCalendarView(calendarHolder, cityNow, monthAnchor);
        }
        cityInput.KeyDown += (_, args) => { if (args.Key == Key.Return) ReloadCalendar(); };
        refreshBtn.Click += (_, _) => ReloadCalendar();
        ReloadCalendar();
        ...
        hourlyToggleBtn.Click += async (s, e) =>
        {
            ...
            if (hourlyExpanded)
            {
                rightCol.Width = new GridLength(WxPaneWidth(win));
                hourlyScroll.Visibility = Visibility.Visible;
                win.Width = WxTotalWidth(win, favExpanded, true);
            ...
            else
            {
                ...
                win.Width = WxTotalWidth(win, favExpanded, false);
                hourlyToggleBtn.Content = ">  Today — Hourly";
            }
        ...
        hourlyToggleBtn.Background = new SolidColorBrush(WxSurface);
        hourlyToggleBtn.Foreground = new SolidColorBrush(WeatherBridge.ThemeAccentSubtle);
        hourlyToggleBtn.BorderBrush = new SolidColorBrush(WxBorder);
        hourlyToggleBtn.ContextMenu = MakeDefaultOpenMenu(
        ...
        async Task<bool> LoadForecastTab(ScrollViewer sv, int days)
        {
            var sp = (StackPanel)sv.Content;
            sp.Children.Clear();
            sp.Children.Add(new TextBlock { Text = "Loading…", Foreground = new SolidColorBrush(WxMuted), FontSize = 11, Margin = new Thickness(0, 6, 0, 6) });
            string fc = cityInput.Text.Trim();
            if (string.IsNullOrEmpty(fc))
            {
                sp.Children.Clear();
                sp.Children.Add(new TextBlock { Text = "Enter a city on the Now tab.", Foreground = new SolidColorBrush(WxMuted), FontSize = 11, Margin = new Thickness(0, 6, 0, 6) });
                return false;
            }
            ...
            if (data.Count > 1)
            {
                ...
                sp.Children.Add(new Border { Height = 1, Background = new SolidColorBrush(WxBorder), Margin = new Thickness(0, 4, 0, 6) });
            }
            var rowsHolder = new StackPanel();
            sp.Children.Add(rowsHolder);
            BuildForecastRows(rowsHolder, data);
            if (data.Count == 0)
            {
                var retryBtn = AccentButton("↻  Retry", WxButtonBg, WxButtonBorder, 110);
                retryBtn.HorizontalAlignment = HorizontalAlignment.Left;
                retryBtn.Margin = new Thickness(0, 8, 0, 0);
                retryBtn.Click += async (_, _) => await LoadForecastTab(sv, days);
        ...
        var bodyRoot = new Grid();
        bodyRoot.ColumnDefinitions.Add(favCol);
        bodyRoot.ColumnDefinitions.Add(new ColumnDefinition { Width = new GridLength(1, GridUnitType.Star) });
        bodyRoot.ColumnDefinitions.Add(rightCol);
        Grid.SetColumn(contentHolder, 1);
        bodyRoot.Children.Add(contentHolder);
        var favSlide = new TranslateTransform();
        favScroll.RenderTransform = favSlide;
        bodyRoot.Children.Add(favScroll);
        hourlyScroll.BorderBrush = new SolidColorBrush(WxBorder);
        hourlyScroll.Background = new SolidColorBrush(Color.FromArgb(0x38, 0x00, 0x00, 0x00));
        var paneSlide = new TranslateTransform();
        hourlyScroll.RenderTransform = paneSlide;
        bodyRoot.Children.Add(hourlyScroll);
        outerDock.Children.Add(bodyRoot);

        hourlyToggleBtn.Click += (_, _) =>
        {
            if (hourlyExpanded)
            {
                paneSlide.BeginAnimation(TranslateTransform.XProperty,
                    new DoubleAnimation(WxPaneWidth(win) * 0.5, 0, TimeSpan.FromMilliseconds(240))
                    {
                        EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut }
                    });
                hourlyScroll.BeginAnimation(UIElement.OpacityProperty,
                    new DoubleAnimation(0, 1, TimeSpan.FromMilliseconds(240)));
            }
            else
            {
                paneDate = DateTime.Today;
                paneUseLive = true;
                hourlyLoaded = false;
            }
        };

        async Task OpenPaneForDay(DateTime date, int code, double max, double min)
        {
            paneDate = date.Date;
            paneCode = code;
            paneMax = max;
            ...
            {
                hourlyLoaded = false;
                hourlyToggleBtn.RaiseEvent(new RoutedEventArgs(Button.ClickEvent));
                return;
            }
            hourlyLoaded = true;
            await LoadTodayHourly();
        ...
        Func<DateTime, int, double, double, Task> paneOpener = OpenPaneForDay;
        _weatherPaneOpener = paneOpener;
        win.Closed += (_, _) =>
        {
            if (ReferenceEquals(_weatherPaneOpener, paneOpener)) _weatherPaneOpener = null;
        };

        bool wxUserAdjusted = false;
        bool wxProgrammaticResize = false;

        void WxBeginProgrammaticResize()
        {
            wxProgrammaticResize = true;
            win.Dispatcher.BeginInvoke(new Action(() => wxProgrammaticResize = false), System.Windows.Threading.DispatcherPriority.Background);
        }
        ...
        void SelectTab(int idx)
        {
            for (int i = 0; i < panels.Length; i++)
            {
                ...
                WxTabStyle(tabBtns[i], i == idx);
            }
        }

        void ApplyTabStretch()
        {
            if (tabBtns.Length < 2) return;
            if (wxUserAdjusted || win.WindowState != WindowState.Normal)
            {
                foreach (var b in tabBtns) b.Margin = new Thickness(0);
                return;
            }
            tabBar.UpdateLayout();
            double sum = 0;
        ...
        for (int i = 0; i < labels.Length; i++)
        {
            ...
            btn.Click += async (s, e) =>
            {
                ...
                if (loaded[ci]) return;
                loaded[ci] = true;
                if      (ci == 1) loaded[ci] = await LoadForecastTab(panel7,   7);
                else if (ci == 2) loaded[ci] = await LoadForecastTab(panel16, 16);
        ...
        loaded[0] = true;
        tabBar.SizeChanged += (_, _) => ApplyTabStretch();
        ApplyTabStretch();

        async Task ReloadTabAsync(int idx)
        {
            if (idx == 1) loaded[1] = await LoadForecastTab(panel7, 7);
            else if (idx == 2) loaded[2] = await LoadForecastTab(panel16, 16);
            else if (idx == 3) { await LoadMonthlyTab(panelMon); loaded[3] = true; }
            else if (idx == 4) { await LoadYearlyTab(panelYr); loaded[4] = true; }
        }
        ...
        void WxLiveReapply()
        {
            ...
            cityInput.BorderBrush = new SolidColorBrush(WxBorder);
            refreshBtn.Background = new SolidColorBrush(WxButtonBg);
            refreshBtn.BorderBrush = new SolidColorBrush(WxButtonBorder);
            hourlyToggleBtn.Background = new SolidColorBrush(WxSurface);
            hourlyToggleBtn.BorderBrush = new SolidColorBrush(WxBorder);
            hourlyScroll.BorderBrush = new SolidColorBrush(WxBorder);
            RebuildFavorites();
            ...
            for (int i = 0; i < tabBtns.Length; i++)
            {
                bool active = panels[i].Visibility == Visibility.Visible;
                WxTabStyle(tabBtns[i], active);
                if (active) visibleIdx = i;
                if (i != 0 && !active) loaded[i] = false;
            }
            if (visibleIdx != 0) _ = ReloadTabAsync(visibleIdx);
        }
        WeatherBridge.ThemeUpdated += WxLiveReapply;
        win.Closed += (_, _) => WeatherBridge.ThemeUpdated -= WxLiveReapply;

        win.SizeToContent = SizeToContent.Manual;
        win.MaxHeight = double.PositiveInfinity;
        win.Height = Math.Min(760, SystemParameters.WorkArea.Height - 40);
        win.MinWidth = 380;
        ...
        var titleBar = new Grid { Height = 34, Background = new SolidColorBrush(Color.FromArgb(0x50, 0x00, 0x00, 0x00)) };
        ...
        var captionButtons = new StackPanel { Orientation = Orientation.Horizontal };
        Grid.SetColumn(captionButtons, 1);
        titleBar.Children.Add(captionButtons);

        (Border Box, TextBlock Glyph) MakeCaptionButton(string glyph, bool danger, Action onClick)
        {
            var glyphTb = new TextBlock
            {
                Text = glyph, FontFamily = new FontFamily("Segoe MDL2 Assets"), FontSize = 10,
                Foreground = Brushes.White,
                HorizontalAlignment = HorizontalAlignment.Center, VerticalAlignment = VerticalAlignment.Center
            };
            var box = new Border { Width = 46, Height = 34, Background = Brushes.Transparent, Cursor = Cursors.Hand, Child = glyphTb };
            var hover = danger ? Color.FromRgb(0xC4, 0x2B, 0x1C) : Color.FromArgb(0x30, 0xFF, 0xFF, 0xFF);
            box.MouseEnter += (_, _) => box.Background = new SolidColorBrush(hover);
            box.MouseLeave += (_, _) => box.Background = Brushes.Transparent;
            box.MouseLeftButtonUp += (_, _) => onClick();
            System.Windows.Shell.WindowChrome.SetIsHitTestVisibleInChrome(box, true);
            captionButtons.Children.Add(box);
            return (box, glyphTb);
        }
        ...
        void ToggleFavPane()
        {
            ...
            if (favExpanded)
            {
                favCol.Width = new GridLength(WxFavPaneWidth(win));
                favScroll.Visibility = Visibility.Visible;
                if (win.WindowState == WindowState.Normal)
                    win.Width = WxTotalWidth(win, true, hourlyExpanded);
                favSlide.BeginAnimation(TranslateTransform.XProperty,
                    new DoubleAnimation(-WxFavPaneWidth(win) * 0.5, 0, TimeSpan.FromMilliseconds(240))
                    {
                        EasingFunction = new CubicEase { EasingMode = EasingMode.EaseOut }
                    });
                favScroll.BeginAnimation(UIElement.OpacityProperty,
                    new DoubleAnimation(0, 1, TimeSpan.FromMilliseconds(240)));
            }
            else
            {
                favScroll.Visibility = Visibility.Collapsed;
                favCol.Width = new GridLength(0);
                if (win.WindowState == WindowState.Normal)
                    win.Width = WxTotalWidth(win, false, hourlyExpanded);
            }
        }
        ContextMenu MakeDefaultOpenMenu(Func<bool> getter, Action<bool> setter)
        {
            var mi = new MenuItem
            {
                Header = "Open by default in Wide mode",
                IsCheckable = true,
                IsChecked = getter(),
                Foreground = Brushes.White,
            ...
            var cm = new ContextMenu { Background = new SolidColorBrush(WxPanel), BorderBrush = new SolidColorBrush(WxBorder) };
            cm.Items.Add(mi);
            return cm;
        }

        var (favBox, _) = MakeCaptionButton("\uE734", false, ToggleFavPane);
        favBox.ContextMenu = MakeDefaultOpenMenu(
            () => SettingsService.Current.WeatherLeftPaneDefaultOpen,
            v => SettingsService.Current.WeatherLeftPaneDefaultOpen = v);
        var (paneBox, paneGlyph) = MakeCaptionButton("\uE76B", false, () => hourlyToggleBtn.RaiseEvent(new RoutedEventArgs(Button.ClickEvent)));
        ...
        win.StateChanged += (_, _) =>
        {
            ...
            win.BorderThickness = maximized ? SystemParameters.WindowResizeBorderThickness : new Thickness(0);
            maxGlyph.Text = maximized ? "\uE923" : "\uE922";
            ApplyTabStretch();
        };
        win.Closed += (_, _) => backdrop.Detach();
        ...
        layoutBox.Width = 76;
        layoutBox.ToolTip = "Layout: click to switch Auto / Compact / Wide";
        layoutGlyph.FontFamily = new FontFamily("Segoe UI");
        layoutGlyph.FontSize = 11;

        string LayoutMode()
        {
            string m = SettingsService.Current.WeatherLayoutMode;
            return m == "Compact" || m == "Wide" ? m : "Auto";
        }
        ...
        bool ResolveWide()
        {
            ...
            bool wasWide = win.Tag is true;
            if (w <= 0) return wasWide;
            return wasWide ? w >= WxWideExit : w >= WxWideEnter;
        }

        void ApplyLayout(bool resizeWindow)
        {
            if (layoutApplying) return;
            layoutApplying = true;
            try
            {
                bool wide = ResolveWide();
                bool changed = (win.Tag is true) != wide;
                win.Tag = wide;
                wxScale.ScaleX = WxScale(win);
                wxScale.ScaleY = WxScale(win);
                layoutGlyph.Text = LayoutMode();
                if (changed && wide)
                {
                    if (!hourlyExpanded && SettingsService.Current.WeatherRightPaneDefaultOpen)
                        hourlyToggleBtn.RaiseEvent(new RoutedEventArgs(Button.ClickEvent));
                    if (!favExpanded && SettingsService.Current.WeatherLeftPaneDefaultOpen)
                        ToggleFavPane();
                }
                if (changed && hourlyExpanded) rightCol.Width = new GridLength(WxPaneWidth(win));
                if (changed && favExpanded) favCol.Width = new GridLength(WxFavPaneWidth(win));
                if (resizeWindow && changed && win.WindowState == WindowState.Normal)
                {
                    WxBeginProgrammaticResize();
                    win.Width = WxTotalWidth(win, favExpanded, hourlyExpanded);
                }
            }
            finally
            {
        ...
        win.Tag = LayoutMode() == "Wide";
        WxBeginProgrammaticResize();
        win.Width = WxTotalWidth(win, false, false);
        ApplyLayout(false);
        layoutBox.MouseLeftButtonUp += (_, _) =>
        {
            wxUserAdjusted = true;
            string current = LayoutMode();
            SettingsService.Current.WeatherLayoutMode = current == "Auto" ? "Compact" : current == "Compact" ? "Wide" : "Auto";
            SettingsService.Save();
            ApplyLayout(true);
            ApplyTabStretch();
        };
        win.SizeChanged += (_, _) =>
        {
            if (!wxProgrammaticResize && win.WindowState == WindowState.Normal) wxUserAdjusted = true;
            ApplyLayout(false);
            ApplyTabStretch();
        };
    ...
    private async Task EnsureWeatherGeoAsync(string city)
{
    for (int attempt = 0; attempt < 3; attempt++)
    {
        await EnsureWeatherGeoOnceAsync(city);
        if (_cachedWeatherCity == city && _cachedWeatherLat != 0) return;
        await Task.Delay(400 * (attempt + 1));
    }
}

private async Task EnsureWeatherGeoOnceAsync(string city)
    {
        if (_cachedWeatherCity == city && _cachedWeatherLat != 0) return;
        try
        {
        ...
        double width, double height)
    {
        ...
        for (int i = 0; i < days.Count; i++)
        {
            ...
            var topC = WeatherBridge.ThemeAccent;
            var botC = WeatherBridge.ThemeAccentSecondary;
            var fill = new LinearGradientBrush
            {
                StartPoint = new Point(0, 0), EndPoint = new Point(0, 1),
                GradientStops = new GradientStopCollection
                {
                    new GradientStop(Color.FromArgb(alpha, topC.R, topC.G, topC.B), 0),
                    new GradientStop(Color.FromArgb(alpha, botC.R, botC.G, botC.B), 1),
                }
            };
        ...
        var border = new System.Windows.Shapes.Rectangle
        {
            ...
            Stroke = new SolidColorBrush(WxBorder),
        ...
        List<(DateTime Date, int WmoCode, double Max, double Min, double RainPct, double PrecipMm)> days)
    {
        ...
        foreach (var item in days)
        {
            ...
            var restBrush = new SolidColorBrush(alt ? Color.FromArgb(0x28, 0x00, 0x00, 0x00) : Color.FromArgb(0x48, 0x00, 0x00, 0x00));
            ...
            row.MouseLeave += (_, _) => row.Background = restBrush;
            ...
            var rainLbl = new TextBlock
            {
                ...
                FontSize = 10,
                Foreground = new SolidColorBrush(rainPct >= 60 ? WxUvColor(3) : WeatherBridge.ThemeAccentSecondary),
                VerticalAlignment = VerticalAlignment.Center, TextAlignment = TextAlignment.Right
            };
            var arrow = new TextBlock
            {
                Text = "›", FontSize = 14, Foreground = new SolidColorBrush(WxMuted),
                VerticalAlignment = VerticalAlignment.Center
            };
    ...
    private Func<DateTime, int, double, double, Task>? _weatherPaneOpener;

    private async Task OpenWeatherDayDetailAsync(DateTime date, int wmoCode, double max, double min)
    {
        var opener = _weatherPaneOpener;
        if (opener != null) await opener(date, wmoCode, max, min);
    }

    private async Task<Dictionary<int, double>> FetchWeatherHourlyUvAsync(DateTime date)
    {
        var result = new Dictionary<int, double>();
        if (_cachedWeatherLat == 0 || date.Date < DateTime.Today) return result;
        string latS = _cachedWeatherLat.ToString(CultureInfo.InvariantCulture);
        string lonS = _cachedWeatherLon.ToString(CultureInfo.InvariantCulture);
        string tz = Uri.EscapeDataString(_cachedWeatherTz);
        try
        {
            string url = $"https://api.open-meteo.com/v1/forecast?latitude={latS}&longitude={lonS}" +
                         $"&hourly=uv_index&timezone={tz}&start_date={date:yyyy-MM-dd}&end_date={date:yyyy-MM-dd}";
            string json = await _widgetHttp.GetStringAsync(url);
            using var doc = JsonDocument.Parse(json);
            var hourly = doc.RootElement.GetProperty("hourly");
            var times = hourly.GetProperty("time");
            var uv = hourly.GetProperty("uv_index");
            int count = times.GetArrayLength();
            for (int i = 0; i < count; i++)
            {
                if (!DateTime.TryParse(times[i].GetString(), out var t)) continue;
                if (uv[i].ValueKind == JsonValueKind.Number) result[t.Hour] = uv[i].GetDouble();
            }
        }
        catch { }
        return result;
    ...
    private static Color WxTempColor(double t) =>
        t < 10 ? Color.FromRgb(0x7C, 0xC4, 0xFF) :
        t < 18 ? Color.FromRgb(0xCF, 0xEA, 0xFF) :
        t < 25 ? Color.FromRgb(0xFF, 0xD2, 0x7A) :
        ...
        DateTime date, int categoryIndex, Dictionary<int, double> uvByHour)
    {
        ...
        areaPts.Add(new Point(padX, baseY));
        ...
        var baseLine = new System.Windows.Shapes.Rectangle
        {
            Width = width - padX * 2, Height = 1, Fill = new SolidColorBrush(WxBorder)
        };
    ...
    private static void BuildDayPane(StackPanel panel, DateTime date, int code, double max, double min,
        List<(DateTime Time, double Temp, double Precip, double WindKmh, int Wmo, int Humidity)> hourly,
        Dictionary<int, double> uvByHour)
    {
        var mono = new FontFamily("Consolas");
        var mutedBrush = new SolidColorBrush(WxMuted);
        ...
        info.Children.Add(new TextBlock
        {
            Text = $"Hi {max:F1}°C  /  Lo {min:F1}°C", FontFamily = mono, FontSize = 12,
            Foreground = new SolidColorBrush(WeatherBridge.ThemeAccent)
        });
        info.Children.Add(new TextBlock
        {
            Text = date.ToString("dddd, d MMMM"), FontSize = 11, Foreground = mutedBrush
        });
        ...
        if (hourly.Count == 0)
        {
            panel.Children.Add(new TextBlock
            {
                Text = "No hourly data available.", Foreground = mutedBrush, FontSize = 11,
        ...
        {
            bool isNow = date.Date == DateTime.Today && time.Hour == DateTime.Now.Hour;
            var restBrush = new SolidColorBrush(alt ? Color.FromArgb(0x28, 0x00, 0x00, 0x00) : Color.FromArgb(0x48, 0x00, 0x00, 0x00));
            var hoverBrush = new SolidColorBrush(Color.FromArgb(0x30, 0xFF, 0xFF, 0xFF));
            alt = !alt;

            double? tipUv = uvByHour.TryGetValue(time.Hour, out double tipUvVal) ? tipUvVal : (double?)null;
            var row = new Border
            {
                Background = restBrush, CornerRadius = new CornerRadius(4),
                Margin = new Thickness(0, 1, 0, 1), Padding = new Thickness(6, 4, 6, 4),
                BorderThickness = new Thickness(1),
                BorderBrush = isNow ? new SolidColorBrush(WeatherBridge.ThemeAccent) : Brushes.Transparent,
                Cursor = Cursors.Hand,
                ToolTip = $"{time:HH:mm} · {WmoCodeToDescription(hWmo)}\n🌡 {temp:F1}°C  ·  💧 Humidity {hum}%  ·  💨 Wind {wind:F0} km/h  ·  🌧 Precip {prec:F1} mm" +
                          (tipUv.HasValue ? $"  ·  ☀ UV {tipUv:F0}" : "")
            };
            ...
            Grid.SetColumn(tempTb, 2);
            Grid.SetColumn(extra, 3);
            g.Children.Add(timeTb);
            g.Children.Add(iconTb);
            g.Children.Add(tempTb);
            g.Children.Add(extra);

            if (uvByHour.TryGetValue(time.Hour, out double uvValue))
            {
                var badge = new Border
                {
                    Width = 24, Height = 20, CornerRadius = new CornerRadius(4),
                    Background = new SolidColorBrush(WxUvColor(uvValue)),
                    VerticalAlignment = VerticalAlignment.Center,
                    Child = new TextBlock
                    {
                        Text = Math.Round(uvValue).ToString("0"), FontSize = 12, FontWeight = FontWeights.Bold,
            ...
            {
                Text = $"{time:HH:mm} · {WmoCodeToDescription(hWmo)}  ·  🌡 {temp:F1}°C  ·  💧 Humidity {hum}%  ·  💨 Wind {wind:F0} km/h  ·  🌧 Precip {prec:F1} mm" +
                       (tipUv.HasValue ? $"  ·  ☀ UV {tipUv:F0}" : ""),
                FontSize = 11, Foreground = new SolidColorBrush(WeatherBridge.ThemeAccentSubtle),
                Margin = new Thickness(44, 3, 4, 2), TextWrapping = TextWrapping.Wrap,
                Visibility = Visibility.Collapsed
            };
            var rowStack = new StackPanel();
            rowStack.Children.Add(g);
            rowStack.Children.Add(detail);
            row.Child = rowStack;
    ...
    {
        int token = (sp.Tag is int t0 ? t0 : 0) + 1;
        sp.Tag = token;
        sp.Children.Clear();
        ...
        if (!(sp.Tag is int current && current == token)) return;
        var dataDict = data.ToDictionary(d => d.Date.Date, d => d);

        sp.Children.Clear();

        var nav = new Grid { Margin = new Thickness(0, 2, 0, 6) };
        ...
        var prevBtn = NavBtn("‹");
        var nextBtn = NavBtn("›");
        foreach (var nb in new[] { prevBtn, nextBtn })
        {
            nb.Background = new SolidColorBrush(WxSurface);
            nb.BorderBrush = new SolidColorBrush(WxBorder);
        }
        ...
        nav.Children.Add(monthLbl);
        nav.Children.Add(nextBtn);
        sp.Children.Add(nav);

        var dowRow = new UniformGrid { Rows = 1, Columns = 7, Margin = new Thickness(0, 0, 0, 3) };
        string[] dows = { "Mo", "Tu", "We", "Th", "Fr", "Sa", "Su" };
        for (int di = 0; di < 7; di++)
        {
            dowRow.Children.Add(new TextBlock
            {
                Text = dows[di], HorizontalAlignment = HorizontalAlignment.Center, FontSize = 10,
                FontWeight = FontWeights.Bold,
                Foreground = new SolidColorBrush(di >= 5 ? WeatherBridge.ThemeAccent : WxMuted)
            });
        }
        sp.Children.Add(dowRow);

        int startDow = ((int)startDate.DayOfWeek + 6) % 7;
        int weekRows = (startDow + daysInMonth + 6) / 7;
        var dayGrid = new UniformGrid { Rows = weekRows, Columns = 7 };
        for (int i = 0; i < weekRows * 7; i++)
        {
            int dn = i - startDow + 1;
            var cell = new Border { Margin = new Thickness(1), CornerRadius = new CornerRadius(4), MinHeight = 42 };
            if (dn >= 1 && dn <= daysInMonth)
            {
                ...
                var restBrush = new SolidColorBrush(isTd ? WxBlend(WxPillOpaque, WxPanel, 0.35) : WxSurface);
                var hoverBrush = new SolidColorBrush(isTd ? WxBlend(WxPillOpaque, WxPanel, 0.2) : Color.FromArgb(0x40, 0xFF, 0xFF, 0xFF));
                cell.Background = restBrush;
                cell.BorderBrush = new SolidColorBrush(isTd ? WxPillOpaque : WxBorder);
                ...
                content.Children.Add(new TextBlock
                {
                    Text = dn.ToString(), FontSize = 11,
                    FontWeight = isTd ? FontWeights.Bold : FontWeights.Normal,
                    HorizontalAlignment = HorizontalAlignment.Center,
                    Foreground = isTd ? Brushes.White : new SolidColorBrush(isWk ? WeatherBridge.ThemeAccent : Color.FromRgb(0xDD, 0xDD, 0xDD))
                });

                bool hasDay = dataDict.TryGetValue(d2.Date, out var dd);
                if (hasDay)
                {
                    var dayIcon = WeatherIconBlock(dd.WmoCode, 11);
                    dayIcon.HorizontalAlignment = HorizontalAlignment.Center;
                    content.Children.Add(dayIcon);
                    content.Children.Add(new TextBlock
                    {
                        Text = $"{dd.Max:F0}/{dd.Min:F0}", FontSize = 8.5,
                        HorizontalAlignment = HorizontalAlignment.Center,
                        Foreground = new SolidColorBrush(WeatherBridge.ThemeAccentSubtle)
                    });
                }
                cell.Child = content;

                var captureDate = d2;
                int captureCode = hasDay ? dd.WmoCode : 1;
                double captureMax = hasDay ? dd.Max : 0;
                double captureMin = hasDay ? dd.Min : 0;

                cell.MouseEnter += (_, _) => cell.Background = hoverBrush;
                cell.MouseLeave += (_, _) => cell.Background = restBrush;
    ...
    private async Task BuildYearlyView(StackPanel sp, string city, int year)
    {
        ...
        sp.Children.Add(new TextBlock { Text = "Loading yearly data…", Foreground = new SolidColorBrush(WxMuted), FontSize = 11, Margin = new Thickness(0, 0, 0, 8) });
        ...
        for (int m = 1; m <= 12; m++)
        {
            ...
            {
                Background      = new SolidColorBrush(Color.FromArgb(0x38, 0x00, 0x00, 0x00)),
                BorderBrush     = new SolidColorBrush(WxBorder),
                BorderThickness = new Thickness(1), CornerRadius = new CornerRadius(4),
            ...
            var mIconLbl = WeatherIconBlock(domCode, 14, new Thickness(4, 0, 4, 0));
            ...
            var mDaysLbl = new TextBlock
            {
                ...
                Foreground = new SolidColorBrush(WxMuted), FontSize = 10,
                VerticalAlignment = VerticalAlignment.Center, TextAlignment = TextAlignment.Right
            };
            var mArrow = new TextBlock { Text = "›", FontSize = 14, Foreground = new SolidColorBrush(WxMuted), VerticalAlignment = VerticalAlignment.Center };
            ...
            bool expanded = false;
            var mBorderRest = new SolidColorBrush(Color.FromArgb(0x38, 0x00, 0x00, 0x00));
            var mBorderHover = new SolidColorBrush(Color.FromArgb(0x30, 0xFF, 0xFF, 0xFF));
            mBorder.MouseEnter += (_, _) => mBorder.Background = mBorderHover;
            mBorder.MouseLeave += (_, _) => mBorder.Background = mBorderRest;
            mBorder.MouseLeftButtonUp += async (_, _) =>
            {
                ...
                if (expanded)
                {
                    ...
                    subSp.Children.Add(new TextBlock { Text = "Loading…", Foreground = new SolidColorBrush(WxMuted), FontSize = 11 });
    ...
    private void LstWidgetOrder_RightClick(object sender, MouseButtonEventArgs e)
    {
        ...
        if (isWeather)
        {
            ...
            foreach (var layoutMode in new[] { "Auto", "Compact", "Wide" })
            {
                var mode = layoutMode;
                var miLayout = new MenuItem
                {
                    Header      = "Weather layout: " + mode,
                    IsCheckable = true,
                    IsChecked   = SettingsService.Current.WeatherLayoutMode == mode,
                    Foreground  = menuFg,
                    Background  = Brushes.Transparent
                };
                miLayout.Click += (_, _) =>
                {
                    SettingsService.Current.WeatherLayoutMode = mode;
                    SettingsService.Save();
                };
