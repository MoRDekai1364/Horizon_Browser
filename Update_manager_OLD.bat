...
echo =====================================================================
echo    [INFO] Scanning for Horizon.Stealth.exe...
echo.
if exist "Horizon.Stealth.exe" (
...
set "cnt=0"
for /f "delims=" %%F in ('dir /s /b "Horizon.Stealth.exe" 2^>nul') do (
    set /a cnt+=1
    setlocal EnableDelayedExpansion
    for %%C in (!cnt!) do (
        endlocal
...
if "!cnt!"=="0" (
    color 0C
    echo    [CRITICAL] Horizon.Stealth.exe not found anywhere.
...
echo.
if not exist "Horizon.Stealth.exe" (
    color 0C
    echo [CRITICAL] Horizon.Stealth.exe NOT found in Active Directory.
    pause
    exit
)

echo [INFO] Closing Horizon processes...
taskkill /F /IM Horizon.Stealth.exe 2>nul
...
if /i "!launch!"=="Y" (
    echo [INFO] Starting Horizon.Stealth.exe...
    start "" "Horizon.Stealth.exe"
)
exit /b 0
