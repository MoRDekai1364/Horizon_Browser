...
(function () {
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === "textToSpeech") {
      setupOffscreenDocument('off_screen.html').then(() => {
        if (twpConfig.get("textToSpeechService") === "bing") {
          chrome.runtime.sendMessage({
            action: "offscreen_bing_textToSpeech",
            text: request.text,
          ...
          chrome.runtime.sendMessage({
            action: "offscreen_google_textToSpeech",
            text: request.text,
            targetLanguage: request.targetLanguage
          }, response => {
            sendResponse();
      ...
      setupOffscreenDocument('off_screen.html').then(() => {
        chrome.runtime.sendMessage({
          action: "offscreen_google_stopAll"
        });
        chrome.runtime.sendMessage({
          action: "offscreen_bing_stopAll"
        });
  ...
  twpConfig.onReady(async () => {
    twpConfig.onChanged((name, newvalue) => {
      if (name === "ttsSpeed") {
        chrome.runtime.sendMessage({
          action: "offscreen_google_ttsSpeed",
          speed: newvalue
        });
        chrome.runtime.sendMessage({
          action: "offscreen_bing_ttsSpeed",
          speed: newvalue
        });
      } else if (name === "ttsVolume") {
        chrome.runtime.sendMessage({
          action: "offscreen_google_ttsVolume",
          volume: newvalue
        });
        chrome.runtime.sendMessage({
          action: "offscreen_bing_ttsVolume",
          volume: newvalue
        });
    ...
    chrome.runtime.sendMessage({
      action: "offscreen_google_ttsVolume",
      volume: twpConfig.get("ttsVolume")
    });
    chrome.runtime.sendMessage({
      action: "offscreen_bing_ttsVolume",
      volume: twpConfig.get("ttsVolume")
...
//# sourceMappingURL=https://raw.githubusercontent.com/FilipePS/TWP---Source-Maps/main/maps/10.1.5.1/textToSpeech.js.map
