...
let currentTab = null;
let isPremium = false;
let currentView = 'bookmarks'; // Default to bookmarks view in full-screen
...
async function checkPendingBookmark() {
  try {
    ...
    if (result.pendingBookmark) {
      // Validate the pending bookmark URL
      if (isValidUrl(result.pendingBookmark)) {
        // Switch to home view and set the URL
        currentTab = { url: result.pendingBookmark, title: 'Bookmark to Save' };
        pageTitle.textContent = 'Bookmark to Save';
        pageUrl.textContent = result.pendingBookmark;
        switchView('home');
      } else {
        console.warn('Invalid pending bookmark URL:', result.pendingBookmark);
        showStatus('Invalid URL from context menu. Please use a valid http:// or https:// URL.', 'error');
      }
      // Clear the pending bookmark
      chrome.storage.local.remove(['pendingBookmark']);
    }
  } catch (error) {
    console.error('Error checking pending bookmark:', error);
  }
}
...
async function init() {
  ...
  try {
    ...
    if (autoSaveToggle) {
      autoSaveToggle.checked = result.autoSaveEnabled === true;
    }
  } catch (error) {
    console.warn('Failed to load auto-save setting:', error);
  }
  
  // Check for pending bookmark from context menu first
  await checkPendingBookmark();
  
  // Get current tab (if available - might not be in full-screen mode)
  if (!currentTab) {
    try {
      ...
      // In full-screen mode, we might not have access to current tab
      pageTitle.textContent = 'Save It Later';
      pageUrl.textContent = '';
    }
  }
...
if (saveBtn) {
  saveBtn.addEventListener('click', async () => {
    // Get current tab dynamically (in case user navigated)
    let tab;
    try {
      const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
      tab = tabs[0];
    ...
    const capturedTitle = tab.title || 'Untitled';
    ...
    try {
      ...
      chrome.runtime.sendMessage({
        ...
        if (chrome.runtime.lastError) {
          ...
          await _saveBookmarkDirectly(url, capturedTitle, user.uid, idToken);
          return;
        }
...
// Fallback function to save directly in popup (if background script fails)
async function _saveBookmarkDirectly(url, title, userId, idToken) {
  try {
    ...
    try {
      if (saveBtn) {
        saveBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>Fetching preview...';
      }
      
      // Use metadata fetcher utility (Vercel first, Cloud Function fallback)
      if (window.metadataFetcher && window.metadataFetcher.fetchUrlMetadata) {
        ...
        if (metadata.title && metadata.title.trim()) {
          finalTitle = metadata.title.trim();
        }
        ...
        // Fallback if metadata fetcher not available
        console.warn('Metadata fetcher not available, using tab title');
      }
    } catch (metadataError) {
      console.warn('Metadata fetching failed, using tab title:', metadataError);
      // Continue with tab title - user can still save bookmark
    }
    ...
    // Create bookmark data (matching Firestore structure)
    const bookmarkData = {
      t: finalTitle, // title
      u: url, // url
      ty: 'url', // type
    ...
    // Save to Firestore
    await window.firestoreAPI.saveBookmark(bookmarkData, userId, idToken);
    
    if (statusMessage) {
      showStatus('Bookmark saved successfully!', 'success');
    }
    
    // Refresh bookmarks if on bookmarks view (force refresh to show new bookmark)
    if (currentView === 'bookmarks' && bookmarksList) {
      loadBookmarks(true);
    }
    ...
    if (statusMessage) {
      ...
      } else if (error.message && (error.message.includes('Premium') || error.message.includes('User document'))) {
...
if (saveManualUrlBtn && manualUrlInput) {
  ...
  saveManualUrlBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      saveManualUrlBtn.innerHTML = '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"></path></svg>Fetching...';
...
async function loadBookmarks(forceRefresh = false) {
  ...
  try {
    ...
    for (const doc of allDocuments) {
      ...
      // Log if we couldn't find a date (for debugging)
      if (!createdAt) {
