...
public static class SearchHighlight
{
    public static readonly DependencyProperty SourceTextProperty =
        DependencyProperty.RegisterAttached("SourceText", typeof(string), typeof(SearchHighlight), new PropertyMetadata(null, OnChanged));
    public static readonly DependencyProperty QueryProperty =
        DependencyProperty.RegisterAttached("Query", typeof(string), typeof(SearchHighlight), new PropertyMetadata(null, OnChanged));
    ...
    private static void OnChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        Brush accent = tb.TryFindResource("Brush_Accent") as Brush ?? Brushes.Orange;
        
        string normText = text.Replace("-", "").Replace(" ", "").ToLowerInvariant();
        string normQuery = query.Replace("-", "").Replace(" ", "").ToLowerInvariant();

        if (string.IsNullOrEmpty(normQuery) || !normText.Contains(normQuery))
        {
            tb.Inlines.Add(new Run(text));
            return;
        }

        bool[] highlightMap = new bool[text.Length];
        
        int matchNormIndex = normText.IndexOf(normQuery);
        while (matchNormIndex >= 0)
        {
            int currentNorm = 0;
            for (int i = 0; i < text.Length; i++)
            {
                ...
                if (c != '-' && c != ' ')
                {
                    if (currentNorm >= matchNormIndex && currentNorm < matchNormIndex + normQuery.Length)
                        highlightMap[i] = true;
                    currentNorm++;
                }
                else if (currentNorm > matchNormIndex && currentNorm < matchNormIndex + normQuery.Length)
                {
                    highlightMap[i] = true;
                }
            }
            matchNormIndex = normText.IndexOf(normQuery, matchNormIndex + normQuery.Length);
        }
        ...
        for (int i = 0; i < text.Length; i++)
        {
            if (highlightMap[i] != isHighlighting)
            {
                if (i > start)
                {
                    var run = new Run(text.Substring(start, i - start));
                    if (isHighlighting)
                    {
                ...
                start = i;
                isHighlighting = highlightMap[i];
            }
        }

        if (start < text.Length)
        {
            var run = new Run(text.Substring(start));
            if (isHighlighting)
            {
                run.FontWeight = FontWeights.Bold;
                run.Foreground = accent;
...
public static class SearchIndex
{
    public static readonly DependencyProperty LabelProperty =
        DependencyProperty.RegisterAttached("Label", typeof(string), typeof(SearchIndex), new PropertyMetadata(null));
    public static readonly DependencyProperty DescriptionProperty =
        DependencyProperty.RegisterAttached("Description", typeof(string), typeof(SearchIndex), new PropertyMetadata(null));
...
public partial class SettingsWindow : Window
    ...
    private void TxtSettingsSearch_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        e.Handled = true;
    }
    ...
    {
        FilterSettingsRows(TxtSettingsSearch.Text?.Trim() ?? "");
    }

    private static IEnumerable<DependencyObject> GetChildrenAggressive(DependencyObject parent)
    {
        if (parent == null) yield break;

        if (parent is ContentControl cc && cc.Content is DependencyObject cDo) yield return cDo;
        else if (parent is Decorator dec && dec.Child is DependencyObject dChild) yield return dChild;
        else if (parent is Panel p) { foreach (UIElement child in p.Children) yield return child; }
        else if (parent is ItemsControl ic) { foreach (var item in ic.Items) if (item is DependencyObject iDo) yield return iDo; }

        foreach (var child in LogicalTreeHelper.GetChildren(parent))
            if (child is DependencyObject d) yield return d;
    }

    private static IEnumerable<T> FindDescendantsAggressive<T>(DependencyObject root, HashSet<DependencyObject>? visited = null) where T : DependencyObject
    {
        visited ??= new HashSet<DependencyObject>();
        if (root == null || !visited.Add(root)) yield break;

        if (root is T match) yield return match;

        foreach (var child in GetChildrenAggressive(root))
        {
            foreach (var descendant in FindDescendantsAggressive<T>(child, visited))
    ...
    private sealed class SettingsSearchResult
    {
        ...
        public FrameworkElement AnchorElement { get; set; } = null!;
        public FrameworkElement? Control { get; set; }
    }

    private sealed class SettingsSearchGroup
    {
        public string SectionHeader { get; set; } = "";
        public List<SettingsSearchResult> Rows { get; set; } = new();
    }

    private static string FindSectionHeader(FrameworkElement element)
    {
        FrameworkElement? current = element;
        while (current != null)
        {
            Panel? p = current.Parent as Panel ?? LogicalTreeHelper.GetParent(current) as Panel;
            if (p != null)
            {
                ...
                if (index >= 0)
                {
                    ...
                    {
                        if (p.Children[i] is TextBlock tb && (tb.FontWeight == FontWeights.Bold || tb.FontWeight == FontWeights.SemiBold || tb.FontSize >= 14))
                            return tb.Text;
                    }
                }
            }
            current = (current.Parent as FrameworkElement) ?? (LogicalTreeHelper.GetParent(current) as FrameworkElement);
        }
        return "";
    }
    ...
    private static bool IsCoveredByTag(DependencyObject candidate, List<DependencyObject> taggedElements)
    {
        foreach (var tagged in taggedElements)
        {
            if (IsAncestorOf(tagged, candidate) || IsAncestorOf(candidate, tagged))
                return true;
        }
        return false;
    }

    private static string? GetToolTipText(FrameworkElement el)
    {
        if (el.ToolTip is string s) return s;
        if (el.ToolTip is ToolTip t && t.Content is string ts) return ts;
        return null;
    }

    private List<SettingsSearchResult>? _searchIndexCache;

    private void BuildSearchIndex()
    {
        if (_searchIndexCache != null) return;
        _searchIndexCache = new List<SettingsSearchResult>();

        for (int tabIndex = 0; tabIndex < TabsMain.Items.Count; tabIndex++)
        {
            var tab = (TabItem)TabsMain.Items[tabIndex]!;
            var tabName = tab.Header?.ToString() ?? "";
            
            DependencyObject rootToSearch = (tab.Content as DependencyObject) ?? tab;
            var allElements = FindDescendantsAggressive<FrameworkElement>(rootToSearch).ToList();
            var explicitlyTagged = new HashSet<DependencyObject>();

            foreach (var el in allElements)
            {
                string explicitLabel = SearchIndex.GetLabel(el);
                if (!string.IsNullOrWhiteSpace(explicitLabel))
                {
                    explicitlyTagged.Add(el);
                    _searchIndexCache.Add(new SettingsSearchResult {
                        Label = explicitLabel, Description = SearchIndex.GetDescription(el) ?? "",
                        SectionName = FindSectionHeader(el), TabName = tabName, TabIndex = tabIndex,
                        Tab = tab, AnchorElement = el, Control = el
                    });
                }
            }

            foreach (var control in allElements.OfType<ContentControl>())
            {
                if ((control is CheckBox || control is RadioButton || control is Button) && !IsCoveredByTag(control, explicitlyTagged.ToList()))
                {
                    if (control.Content is string contentStr && !string.IsNullOrWhiteSpace(contentStr))
                    {
                        _searchIndexCache.Add(new SettingsSearchResult {
                            Label = contentStr, Description = GetToolTipText(control) ?? "",
                            SectionName = FindSectionHeader(control), TabName = tabName, TabIndex = tabIndex,
                            Tab = tab, AnchorElement = control, Control = control
                        });
                    }
                }
            }

            foreach (var panel in allElements.OfType<Panel>())
            {
                var children = panel.Children.OfType<FrameworkElement>().ToList();
                for (int i = 0; i < children.Count; i++)
                {
                    if (children[i] is TextBlock tb && !string.IsNullOrWhiteSpace(tb.Text) && tb.FontWeight != FontWeights.Bold)
                    {
                        if (IsCoveredByTag(tb, explicitlyTagged.ToList())) continue;

                        Control? targetControl = null;
                        for (int j = i + 1; j < Math.Min(i + 3, children.Count); j++)
                        {
                            if (children[j] is ComboBox || children[j] is TextBox || children[j] is Slider)
                            {
                                targetControl = (Control)children[j];
                                break;
                            }
                            if (children[j] is TextBlock nextTb && (nextTb.FontWeight == FontWeights.Bold || nextTb.FontWeight == FontWeights.SemiBold)) break; 
                        }

                        if (targetControl != null && !IsCoveredByTag(targetControl, explicitlyTagged.ToList()))
                        {
                            _searchIndexCache.Add(new SettingsSearchResult
                            {
                                Label = tb.Text.TrimEnd(':'),
                                Description = GetToolTipText(targetControl) ?? "",
                                SectionName = FindSectionHeader(tb),
                                TabName = tabName, TabIndex = tabIndex,
                                Tab = tab, AnchorElement = tb, Control = targetControl
                            });
                        }
                    }
                }
    ...
    private void FilterSettingsRows(string query)
    {
        ...
        if (!hasQuery)
        {
            ...
            ItemsSearchResults.ItemsSource = null;
            return;
        }
        ...
        foreach (var item in _searchIndexCache!)
        {
            string normLabel = item.Label.Replace("-", "").Replace(" ", "").ToLowerInvariant();
            string normDesc = item.Description.Replace("-", "").Replace(" ", "").ToLowerInvariant();

            int relevance = 0;
            if (item.Label.Equals(query, StringComparison.OrdinalIgnoreCase)) relevance = 110;
            else if (normLabel == normQuery) relevance = 100;
            else if (normLabel.StartsWith(normQuery)) relevance = 75;
            else if (normLabel.Contains(normQuery)) relevance = 50;
            else if (normDesc.Contains(normQuery)) relevance = 25;

            if (relevance > 0)
            {
                var clone = new SettingsSearchResult
                {
                    Label = item.Label,
                    Description = item.Description,
                    SectionName = item.SectionName,
                    TabName = item.TabName,
                    Breadcrumb = string.IsNullOrEmpty(item.SectionName) ? $"found in {item.TabName}" : $"found in {item.TabName} > {item.SectionName}",
                    Query = query,
        ...
        if (results.Count == 0)
        {
            LogService.RecordCrash(new Exception($"ZERO RESULTS for \"{query}\" (Index Size: {_searchIndexCache.Count})"), "SearchZeroResultsDiag");
        }

        var groups = results
            .GroupBy(r => r.TabIndex)
            .OrderBy(g => g.Key)
            .Select(g => new SettingsSearchGroup
            {
                SectionHeader = $"{g.First().TabName} // ({g.Count()} results)",
                Rows = g.OrderByDescending(r => r.Relevance).ToList()
            })
            .ToList();

        ItemsSearchResults.ItemsSource = groups;

        bool hasResults = results.Count > 0;
        TxtResultsSummary.Text = hasResults
            ? $"Showing 1-{results.Count} of {results.Count} results for \"{query}\""
            : "";

        ScrollSearchResults.Visibility = hasResults ? Visibility.Visible : Visibility.Collapsed;
        PnlEmptyState.Visibility = hasResults ? Visibility.Collapsed : Visibility.Visible;
        if (!hasResults)
            TxtEmptyStateDetail.Text = $"Nothing found for \"{query}\". Check the spelling or try a different term.";

        BtnFilterAll.Content = $"ALL ({results.Count})";
        BtnFilterSettings.Content = $"SETTINGS ({results.Count})";

        TabsMain.Visibility = Visibility.Collapsed;
        GridSearchResults.Visibility = Visibility.Visible;
    }

    private void SetSearchFilterActive(bool allActive)
    {
        BtnFilterAll.Background = allActive ? (Brush)FindResource("Brush_Accent") : Brushes.Transparent;
        BtnFilterAll.Foreground = allActive ? Brushes.Black : (Brush)FindResource("Brush_SoftText");
        BtnFilterSettings.Background = !allActive ? (Brush)FindResource("Brush_Accent") : Brushes.Transparent;
        BtnFilterSettings.Foreground = !allActive ? Brushes.Black : (Brush)FindResource("Brush_SoftText");
    }

    private void BtnFilterAll_Click(object sender, RoutedEventArgs e)
    {
        SetSearchFilterActive(allActive: true);
    }

    private void BtnFilterSettings_Click(object sender, RoutedEventArgs e)
    {
        SetSearchFilterActive(allActive: false);
    }

    private void BtnGoToSetting_Click(object sender, RoutedEventArgs e)
    {
        if (sender is not Button btn || btn.Tag is not SettingsSearchResult result)
            return;

        TxtSettingsSearch.Text = "";
        TabsMain.SelectedItem = result.Tab;
        Dispatcher.BeginInvoke(new Action(() =>
        {
            result.AnchorElement.BringIntoView();
            FlashElement(result.AnchorElement);
            result.Control?.Focus();
        }), System.Windows.Threading.DispatcherPriority.Loaded);
    }

    private void FlashElement(FrameworkElement el)
    {
        Color accentColor = ((SolidColorBrush)FindResource("Brush_Accent")).Color;

        if (el is Panel panel)
        {
            Color highlight = Color.FromArgb(90, accentColor.R, accentColor.G, accentColor.B);
        ...
        {
            Color original = (tb.Foreground as SolidColorBrush)?.Color ?? Colors.White;
            var flashBrush = new SolidColorBrush(accentColor);
            tb.Foreground = flashBrush;
    ...
    private void TxtSettingsSearch_KeyDown(object sender, KeyEventArgs e)
    {
        if (e.Key == Key.Escape)
        {
            TxtSettingsSearch.Text = "";
    ...
    private void BtnOpenHomepageSettings_Click(object sender, RoutedEventArgs e)
{
    var win = new HomePageSettingsWindow { Owner = this };
    win.ShowDialog();
}
...
private void SettingsWindow_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        for (int i = 0; i < TabsMain.Items.Count; i++)
        {
            TabsMain.SelectedIndex = i;
            TabsMain.UpdateLayout();
        }
    ...
    private void LoadState()
    {
        ...
        ChkMediaWidgetAutoShowOnMinimize.IsChecked = s.MediaWidgetAutoShowOnMinimize;
        ChkMediaWidgetAutoShowMinimized.IsChecked  = s.MediaWidgetAutoShowMinimized;
        ChkMediaWidgetAutoShowMinimized.IsEnabled   = s.MediaWidgetAutoShowOnMinimize;
        ChkMediaWidgetAutoShowOnMinimize.Checked   += ChkMediaWidgetAutoShowOnMinimize_Changed;
        ChkMediaWidgetAutoShowOnMinimize.Unchecked += ChkMediaWidgetAutoShowOnMinimize_Changed;
    ...
    private void BtnSave_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(s.HomePage) && !s.SavedHomepages.Contains(s.HomePage))
        {
            s.SavedHomepages.Add(s.HomePage);
        }
        ...
        s.MediaWidgetAutoShowOnMinimize = ChkMediaWidgetAutoShowOnMinimize.IsChecked == true;
        s.MediaWidgetAutoShowMinimized  = ChkMediaWidgetAutoShowMinimized.IsChecked  == true;
    ...
    private void ChkMediaWidgetAutoShowOnMinimize_Changed(object sender, RoutedEventArgs e)
    {
        ChkMediaWidgetAutoShowMinimized.IsEnabled = ChkMediaWidgetAutoShowOnMinimize.IsChecked == true;
    }
