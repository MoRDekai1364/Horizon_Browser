...
public static class AccountSyncService
{
    ...
    // ── App-wide OAuth client (mass-distribution mode) ─────────────────────────
    // Every Horizon install shares ONE registered Google OAuth client — this is
    // the same model Spotify/Discord/Notion use, so end users just click
    // "Sign in with Google" with nothing to configure. Create this once at
    // console.cloud.google.com (OAuth client type: Desktop app), then submit the
    // consent screen for verification before public release (see chat notes).
    public const string AppGoogleClientId     = "528359355035-if6ankafbhmcircb9pk4iguguga482li.apps.googleusercontent.com";
    public const string AppGoogleClientSecret = "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";

    // ── Scopes ────────────────────────────────────────────────────────────────

    private const string GoogleCalendarScope = "https://www.googleapis.com/auth/calendar.readonly";
    private const string GoogleTasksScope    = "https://www.googleapis.com/auth/tasks";
    ...
    public static async Task<SyncAccount?> AddGoogleAccountAsync(string clientId, string clientSecret)
    {
        ...
        string scope = Uri.EscapeDataString($"{GoogleProfileScope} {GoogleCalendarScope} {GoogleTasksScope}");
    ...
    // ── Google Tasks (real, public API — replaces the non-existent "Keep API") ──
    // Google has never published a public Keep API. A Google Task's "notes"
    // field maps 1:1 to a Horizon note body, so we use Tasks instead.

    public static async Task<List<SyncNote>> FetchGoogleTasksAsync(SyncAccount account,
        string clientId, string clientSecret)
    {
        ...
        using var req = new HttpRequestMessage(HttpMethod.Get,
            "https://tasks.googleapis.com/tasks/v1/lists/@default/tasks?showHidden=true&maxResults=100");
        req.Headers.Authorization = new AuthenticationHeaderValue("Bearer", account.AccessToken);
        var resp = await Http.SendAsync(req);
        string json = await resp.Content.ReadAsStringAsync();
        if (!resp.IsSuccessStatusCode)
            throw new Exception($"Google Tasks API {resp.StatusCode}: {json[..Math.Min(json.Length, 160)]}");
        using var doc = JsonDocument.Parse(json);

        var notes = new List<SyncNote>();
        if (!doc.RootElement.TryGetProperty("items", out var items)) return notes;
        foreach (var item in items.EnumerateArray())
        {
            notes.Add(new SyncNote
            {
                ...
                Body    = item.TryGetProperty("notes",   out var b)   ? b.GetString()??"" : "",
                Updated = item.TryGetProperty("updated", out var ut) &&
    ...
    public static async Task PushGoogleTaskAsync(SyncAccount account, string clientId, string clientSecret,
        string title, string body)
    {
        if (account.IsExpired)
            await RefreshGoogleTokenAsync(account, clientId, clientSecret);

        var payload = new { title, notes = body };
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            throw new Exception($"Google Tasks API {resp.StatusCode}: {err[..Math.Min(err.Length, 160)]}");
        }
    }
