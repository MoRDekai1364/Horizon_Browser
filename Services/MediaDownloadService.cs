...
public readonly record struct MediaDownloadProgress(long BytesReceived, long TotalBytes, string FilePath);

public sealed class MediaDownloadService : IDisposable
{
    /// <summary>Set as soon as the destination file name/path is resolved, even if the download later fails.</summary>
        ...
        CancellationToken ct = default)
    {
        ...
        DownloadsBridge.Start(_downloadId, url);
        ...
        try
        {
            ...
            if (!ytDlpReady)
            {
                ...
                DownloadsBridge.Fail(_downloadId);
                return false;
            }
            ...
            if (success) DownloadsBridge.Complete(_downloadId, outputDir);
            else DownloadsBridge.Fail(_downloadId);
    ...
    public async Task<bool> DownloadDirectFileAsync(
        string url,
        string mediaKind,
        Action<MediaDownloadProgress>? onProgress = null,
        CancellationToken ct = default)
    {
        InitLog();
        Log($"[START]  Direct download ({mediaKind}) URL: {url}");

        try
        {
            string? outputDir = ResolveOutputDirectory(useConfiguredDownloadsPath: true);
            if (outputDir == null)
            {
                Log("[FATAL] No writable output directory found. User cancelled.");
                DownloadsBridge.Fail(_downloadId);
            ...
            string fileName = ResolveFileName(url, resp, mediaKind);
            string destPath = DedupePath(Path.Combine(outputDir, fileName));
            LastResolvedPath = destPath;
            Log($"[OUTPUT] File: {destPath}");

            long totalBytes = resp.Content.Headers.ContentLength ?? -1;
            long received   = 0;
            DownloadsBridge.Start(_downloadId, Path.GetFileName(destPath));
            onProgress?.Invoke(new MediaDownloadProgress(0, totalBytes, destPath));
            ...
            await using (var output = File.Create(destPath))
            {
                ...
                while ((read = await input.ReadAsync(buffer, ct)) > 0)
                {
                    ...
                    received += read;
                    DownloadsBridge.UpdateProgress(_downloadId, received, totalBytes, destPath);
                    onProgress?.Invoke(new MediaDownloadProgress(received, totalBytes, destPath));
                }
            }

            Log($"[END]    Result: SUCCESS ({received} bytes) -> {destPath}");
            DownloadsBridge.Complete(_downloadId, destPath);
            return true;
        }
    ...
    private static string ResolveFileName(string url, HttpResponseMessage resp, string mediaKind)
    {
        string? name = resp.Content.Headers.ContentDisposition?.FileNameStar
        ...
        {
            string ext = ExtensionFromContentType(resp.Content.Headers.ContentType?.MediaType, mediaKind);
            string baseName = string.IsNullOrWhiteSpace(name)
                ? $"horizon_{mediaKind}_{DateTime.Now:yyyyMMdd_HHmmss}"
                : name;
            name = baseName + ext;
        }
    ...
    private static string ExtensionFromContentType(string? contentType, string mediaKind) => contentType switch
    {
        "image/jpeg"    => ".jpg",
        "image/png"     => ".png",
        "image/gif"     => ".gif",
    ...
    private string? ResolveOutputDirectory(bool useConfiguredDownloadsPath = false)
    {
        string candidate1 = useConfiguredDownloadsPath
            ? SettingsService.Current.DownloadsPath
            : Path.Combine(_appRoot, "temp_downloads");
        Log($"[DISK] Candidate 1 ({(useConfiguredDownloadsPath ? "configured Downloads folder" : "project temp")}): {candidate1}");
        ...
        string candidate2 = useConfiguredDownloadsPath
            ? Path.Combine(_appRoot, "temp_downloads")
            : Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), "Downloads");
        Log($"[DISK] Candidate 2 ({(useConfiguredDownloadsPath ? "project temp" : "OS Downloads")}): {candidate2}");
        ...
        proc.OutputDataReceived += (_, e) =>
        {
            ...
            if (match.Success &&
                double.TryParse(
                    ...
                    out double pct))
            {
                DownloadsBridge.UpdatePercent(_downloadId, pct);
