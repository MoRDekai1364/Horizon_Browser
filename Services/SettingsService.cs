...
public static class SettingsService
{
    ...
    // Shared source of truth for the built-in engines, so SettingsWindow and HomePageView
    // never fall out of sync. Mirrors the ComboBoxItems in SettingsWindow.xaml (CboSearch).
    public static readonly List<SearchEngineEntry> BuiltInSearchEngines = new()
    {
        new SearchEngineEntry { Name = "AlohFind",    Url = "https://alohafind.com/search/?q={query}",   BuiltIn = true },
        new SearchEngineEntry { Name = "DuckDuckGo",  Url = "https://duckduckgo.com/?q={query}",         BuiltIn = true },
        new SearchEngineEntry { Name = "Google",      Url = "https://www.google.com/search?q={query}",  BuiltIn = true },
        new SearchEngineEntry { Name = "Bing",        Url = "https://www.bing.com/search?q={query}",    BuiltIn = true },
...
public class LayoutSet
{
    ...
    public LayoutProfile NormalLayout { get; set; } = new();
    public LayoutProfile InactivityLayout { get; set; } = new();
}
...
public class SettingsData
{
    ...
    public List<string> SavedHomepages { get; set; } = new();
    
    public string Theme { get; set; } = "Horizon";
    public string BackgroundImage { get; set; } = ""; 
    public double BackgroundOpacity { get; set; } = 1.0;
    public string WeatherLayoutMode { get; set; } = "Auto"; 

    public string       WallpaperMode        { get; set; } = "Random";
    public string       WallpaperFileName    { get; set; } = "";
    public List<string> WallpaperCustomList  { get; set; } = new();
    public string       WallpaperOrder       { get; set; } = "Sequential";
    public int          WallpaperCustomIndex { get; set; } = -1;
    public string       WallpaperFolderPath  { get; set; } = "";

    public bool HomeShowClockWeatherSection { get; set; } = true;
    public bool HomeShowSearchSection       { get; set; } = true;

    public bool HomeShowClock           { get; set; } = true;
    public bool HomeShowWeather         { get; set; } = true;
    public bool HomeShowMediaWidget     { get; set; } = true;
    public bool HomeShowCalendarWidget  { get; set; } = true;
    public bool HomeShowDownloadsWidget { get; set; } = true; 
    ...
    public bool         HomeVisualizerEnabled   { get; set; } = false;
    ...
    public bool HideSponsoredResults    { get; set; } = true;
    ...
    public List<string> WeatherFavoriteCities { get; set; } = new();
    public bool          WeatherRightPaneDefaultOpen { get; set; } = true;
    public bool          WeatherLeftPaneDefaultOpen  { get; set; } = false;
    ...
    // ── Zoom ─────────────────────────────────────────────────────────────────
    // 1.0 = 100%. Applied to new tabs and live-applied to the current tab on save.
    ...
    // When true, a registry Run key launches Horizon minimized/hidden to tray
    // at Windows login. See StartupService.
    public bool StartOnSystemStartup { get; set; } = false;

    // ── Startup splash ───────────────────────────────────────────────────────
    public bool ShowStartupVideo { get; set; } = true;
    public double StartupVideoCachedWidth  { get; set; } = 0;
    public double StartupVideoCachedHeight { get; set; } = 0;
    public string StartupVideoBgColorHex   { get; set; } = "#0A1A3C";
    public double StartupVideoWarmupMs     { get; set; } = -1;

    // "Fixed" | "Random" | "Custom"
    public string       StartupVideoMode        { get; set; } = "Fixed";
    public string       StartupVideoFileName     { get; set; } = "Animate_this_image.mp4";
    public List<string>  StartupVideoCustomList   { get; set; } = new();
    // "Sequential" | "Random" — only applies to Custom mode
    public string       StartupVideoOrder         { get; set; } = "Sequential";
    public int          StartupVideoCustomIndex   { get; set; } = -1;
}