...
public partial class BrowserView : UserControl
{
    ...
    private bool _processCrashed = false;
    public bool IsProcessCrashed => _processCrashed;
    ...
    private void CoreWebView2_PermissionRequested(object? sender, CoreWebView2PermissionRequestedEventArgs e)
    {
        ...
        Dispatcher.InvokeAsync(() =>
        {
            try
            {
                // Auto-decide, no blocking modal dialog — a modal MessageBox here
    ...
    private async void InitializeAsync()
    {
        ...
        try
        {
            ...
            if (StealthEnvironment.Instance != null)
            {
                ...
                MainWebView.CoreWebView2.WebResourceRequested += (s, e) =>
                {
                    e.Request.Headers.RemoveHeader("Sec-CH-UA");
                    e.Request.Headers.RemoveHeader("Sec-CH-UA-Mobile");
                    e.Request.Headers.RemoveHeader("Sec-CH-UA-Platform");
                    if (!e.Request.Headers.Contains("Accept"))
                        e.Request.Headers.SetHeader("Accept", "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8");
                    if (!e.Request.Headers.Contains("Accept-Language"))
                        e.Request.Headers.SetHeader("Accept-Language", "en-US,en;q=0.9");
                };
                MainWebView.CoreWebView2.AddWebResourceRequestedFilter("*", CoreWebView2WebResourceContext.Document);
                ...
                MainWebView.CoreWebView2.ProcessFailed += (s, e) =>
                {
                    ...
                    Dispatcher.Invoke(async () =>
                    {
                        try
                        {
                            LogService.Write("CRASH", $"BrowserView ProcessFailed: Kind={e.ProcessFailedKind} Reason={e.Reason}");
                            if (e.ProcessFailedKind == CoreWebView2ProcessFailedKind.BrowserProcessExited)
                                await RecoverFromCrashAsync();
                            else
                            {
                                _processCrashed = false;
                                try { MainWebView.Reload(); } catch { }
                            }
                        }
                        catch (Exception ex)
                        {
                            LogService.RecordCrash(ex, "BrowserView.ProcessFailed handler");
                        }
                    });
                };
    ...
    private async void CoreWebView2_NavigationCompleted(object? sender, CoreWebView2NavigationCompletedEventArgs e)
    {
        ...
        // Fix: hardware-accelerated video (e.g. YouTube Music watch pages) can init
        // with audio playing but a black/uncomposited frame when the WebView2 surface
        // wasn't already "warm" (i.e. direct/deep-link navigation). Nudging the size
        // forces WPF to recomposite the WebView2 visual instead of leaving it black.
        Dispatcher.InvokeAsync(() => ForceWebViewRepaint(), System.Windows.Threading.DispatcherPriority.Background);
    ...
    private async void ForceWebViewRepaint()
    {
        try
        {
            // Retry over a longer window — pages that arrive via a redirect
            // chain (e.g. YouTube's consent-screen bounce before landing on
            // music.youtube.com/watch) mount their canvas/video well after
            // NavigationCompleted fires, so a couple of quick resize events
            // right away can land before there's anything to repaint.
            int[] delaysMs = { 250, 500, 1000, 2000, 3500 };
            foreach (var delay in delaysMs)
            {
                ...
                await core.ExecuteScriptAsync("window.dispatchEvent(new Event('resize'));");

                // Native nudge: toggling WPF Visibility forces WPF to fully
                // recompose the WebView2 airspace/hosted HWND, which is a
                // stronger repaint than a JS-level resize event alone.
                Dispatcher.Invoke(() =>
                {
        ...
        {
            LogService.RecordCrash(ex, "ForceWebViewRepaint");
        }
    }
...
"));
    ...
    public const string HomeSentinel = "horizon://home";
    ...
    public void Navigate(string url)
    {
        ...
        if (MainWebView == null || _processCrashed)
        {
            _pendingUrl = url;
            return;
        }
        ...
        {
            LogService.Write("CRASH", "Navigate() hit a dead WebView2 process, queuing reinit.");
            _processCrashed = true;
            _pendingUrl = url;
            _ = RecoverFromCrashAsync();
        }
    }

    private async Task RecoverFromCrashAsync()
    {
        try
        {
            await Task.Delay(300);

            string? urlToRestore = _pendingUrl;
            if (string.IsNullOrEmpty(urlToRestore))
            {
                try { urlToRestore = MainWebView.Source?.ToString(); } catch { }
            }
            _pendingUrl = null;

            if (MainWebView.Parent is Panel parentPanel)
            {
                int index = parentPanel.Children.IndexOf(MainWebView);

                try { MainWebView.Dispose(); } catch { }

                var freshWebView = new Microsoft.Web.WebView2.Wpf.WebView2();
                Grid.SetRow(freshWebView, Grid.GetRow(MainWebView));

                if (index >= 0)
                {
                    parentPanel.Children.RemoveAt(index);
                    parentPanel.Children.Insert(index, freshWebView);
                }
                else
                {
                    parentPanel.Children.Remove(MainWebView);
                    parentPanel.Children.Add(freshWebView);
                }

                MainWebView = freshWebView;
            }
            else
            {
                try { MainWebView.Dispose(); } catch { }
            ...
            if (!string.IsNullOrEmpty(urlToRestore))
                _pendingUrl = urlToRestore;

            InitializeAsync();
        }
        catch (Exception ex)
        {
            LogService.RecordCrash(ex, "BrowserView.RecoverFromCrashAsync");
        }
    }
}